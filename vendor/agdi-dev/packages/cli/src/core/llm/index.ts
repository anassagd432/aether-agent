...
import type { ILLMProvider, LLMConfig, LLMResponse, LLMProviderType, ChatMessage } from '../types/index.js';

export { ILLMProvider, LLMConfig, LLMResponse, LLMProviderType, ChatMessage };
...
export class GeminiProvider implements ILLMProvider {
    ...
    async chat(messages: ChatMessage[]): Promise<LLMResponse> {
        ...
        const contents = messages.map(m => {
            if (m.role === 'system') {
                // Gemini handles system as user+model pair
...
/**
 * OpenRouter LLM Provider - Access 100+ models with single API key
 * https://openrouter.ai/
 */
